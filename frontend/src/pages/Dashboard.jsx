...
function DailyCheckIn() {
  ...
  const OPTIONS = [
    { label: 'Amazing', emoji: '✨', color: '#a8e6cf', icon: Star },
    { label: 'Good', emoji: '😊', color: '#d1f2ff', icon: Sun },
    { label: 'Okay', emoji: '😐', color: '#ffe5a0', icon: Cloud },
    { label: 'Tough', emoji: '😔', color: '#ffe0e0', icon: CloudRain },
  ];
  ...
  if (status === 'submitted') {
    return (
      <div className="glass animate-fade-up" style={{ padding: 32, borderRadius: 20, textAlign: 'center', background: 'rgba(168,230,207,0.15)', border: '1px solid rgba(168,230,207,0.4)', marginBottom: 28 }}>
        <div style={{ display: 'inline-flex', padding: 12, borderRadius: '50%', background: 'rgba(168,230,207,0.2)', marginBottom: 16 }}>
           <CheckCircle size={32} color="#5cb89c" />
        </div>
  ...
  return (
    <div className="glass animate-fade-up" style={{ padding: 24, borderRadius: 20, marginBottom: 28, border: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(235,177,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {OPTIONS.map(opt => (
          <button 
            ...
            style={{ 
              ...
              border: '1px solid var(--border)', background: 'rgba(255,255,255,0.6)', borderRadius: 16,
...
export default function Dashboard() {
  ...
  const COMMUNITY_HIGHLIGHTS = [
    { id: 1, title: 'Music Group', members: 124, lastMsg: 'I love this song!', icon: Headphones, color: '#dadaff' },
    { id: 2, title: 'Gaming Hub', members: 89, lastMsg: 'Anyone up for a match?', icon: PlayCircle, color: '#d1f2ff' },
  ];

  const RECOMMENDED_ACTIVITIES = [
    { title: 'Box Breathing', desc: '4-4-4-4 to center yourself', icon: Wind, color: '#a8e6cf', path: '/stress-relief' },
    { title: 'Gratitude Journal', desc: 'Write 3 things you love', icon: BookOpen, color: '#ffc6e9', path: '/mood' },
  ];
  ...
  return (
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        {/* Welcome Back Card */}
        <div className="glass animate-fade-up" style={{ padding: 32, marginBottom: 28, borderRadius: 20, background: 'linear-gradient(135deg, rgba(235,177,255,0.15), rgba(209,242,255,0.15))', border: '1px solid rgba(235,177,255,0.3)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                {['😊', '😐', '😔', '😣', '✨'].map(emoji => (
                  <button key={emoji} onClick={() => setSelectedEmoji(emoji)} style={{ fontSize: 24, padding: '12px 16px', borderRadius: 12, background: selectedEmoji === emoji ? 'rgba(235,177,255,0.15)' : 'transparent', border: `1px solid ${selectedEmoji === emoji ? 'rgba(235,177,255,0.3)' : 'transparent'}`, cursor: 'pointer', transition: 'all 0.2s' }}>{emoji}</button>
                ))}
              </div>
            </div>
            <div style={{ padding: 16, borderRadius: '50%', background: 'rgba(255,198,233,0.15)' }}>
        ...
        <div style={{ padding: '16px 20px', borderRadius: 14, background: 'rgba(218,218,255,0.1)', border: '1px solid var(--border)', marginBottom: 28 }} className="animate-fade-up delay-100">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <Zap size={16} color="#9B8FD8" />
