        ...
        {!selected ? (
          <div>
            <div style={{ marginBottom: 28 }}>
              <h1 className="section-title"><Users size={22} style={{ display: 'inline', marginRight: 10, color: '#7ABCD4' }} />Volunteer Mentors</h1>
            ...
            {moodData?.stressLevel >= 7 && (
              <div className="crisis-banner animate-fade-up" style={{ marginBottom: 28, background: 'linear-gradient(135deg, rgba(239,68,68,0.1), rgba(220,38,38,0.05))', border: '1px solid rgba(239,68,68,0.3)' }}>
                <AlertTriangle size={22} color="#B85C5C" style={{ flexShrink: 0, marginTop: 2 }} />
                <div>
                  <div style={{ fontWeight: 800, color: '#B85C5C', marginBottom: 2 }}>Immediate support recommended</div>
                  ...
                  <div key={mentor._id} className="mentor-card glass">
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 52, height: 52, borderRadius: 16, background: 'linear-gradient(135deg, #d1f2ff, #dadaff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 800, color: '#2D2B55' }}>
                          {mentor.name[0]}
                        </div>
                        <div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                               <Star size={12} fill="#C4A43D" color="#C4A43D" />
                               <span style={{ fontSize: 13, color: '#9B7A1E', fontWeight: 700 }}>{mentor.rating}</span>
                            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-primary)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 16, background: 'var(--bg-secondary)', zIndex: 10 }}>
              ...
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, #d1f2ff, #dadaff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, color: '#2D2B55' }}>{selected.name[0]}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 16 }}>Talking to {selected.name}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#5cb89c' }} className="animate-pulse" />
                  <span style={{ fontSize: 11, color: '#5cb89c', fontWeight: 600 }}>Active Session</span>
                </div>
              </div>
            </div>

            <div style={{ flex: 1, overflowY: 'auto', padding: '32px', display: 'flex', flexDirection: 'column', gap: 24, background: 'rgba(235,177,255,0.03)' }}>
