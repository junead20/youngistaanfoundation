      ...
      <div className="page-content animate-fade-up">
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{
            ...
            background: 'rgba(168,230,207,0.2)', border: '1px solid rgba(168,230,207,0.5)',
            borderRadius: 999, marginBottom: 16
          }}>
            <Shield size={14} color="#5cb89c" />
            <span style={{ fontSize: 12, fontWeight: 600, color: '#3B8B6E' }}>100% Anonymous — No Email Required</span>
          </div>
          <h1 style={{ fontSize: 26, fontWeight: 800, marginBottom: 8 }}>Enter Reachus</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Your identity stays completely private</p>
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, padding: 4, background: 'rgba(218,218,255,0.15)', borderRadius: 12 }}>
          {[{ id: 'new', label: '✨ First Time', icon: User }, { id: 'returning', label: '🔑 Return User', icon: Hash }].map(t => (
            <button
              ...
              style={{
                ...
                background: tab === t.id ? 'linear-gradient(135deg, #ebb1ff, #dadaff)' : 'transparent',
                color: tab === t.id ? '#2D2B55' : 'var(--text-secondary)',
                boxShadow: tab === t.id ? '0 4px 16px rgba(235,177,255,0.4)' : 'none',
              }}
            >
              {t.label}
            </button>
        ...
        <div className="glass" style={{ padding: 28 }}>
          {tab === 'new' ? (
            <div className="animate-fade">
              ...
              {error && <p style={{ color: '#B85C5C', fontSize: 13, marginBottom: 16 }}>{error}</p>}
              <button
