    ...
    <div style={{
      ...
      {!isOwn && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, marginLeft: 4 }}>
          <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'linear-gradient(135deg, #d1f2ff, #dadaff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#2D2B55' }}>
            {message.sender[0]}
          </div>
      ...
      <div className={`chat-bubble ${isOwn ? 'chat-bubble-user' : 'chat-bubble-ai'}`} style={{
        ...
        background: isOwn ? 'linear-gradient(135deg, var(--purple-primary), var(--purple-light))' : 'rgba(255,255,255,0.8)',
